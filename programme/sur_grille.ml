...
let dist_euclidienne (a,b) (x,y) = sqrt (float_of_int ((x-a)*(x-a) + (y-b)*(y-b)))

let dis_to_exit ld (a,b) =
	let minab = ref (Float.infinity) in 
	List.iter (fun (xd,yd) -> 
			let tmp1 = dist_euclidienne (a,b) (xd,yd) in
...
let resolution (p : plateau) (x : int) (y : int) (start_coord : (int * int)) (ld : (int * int) list) : (int* int) list=
	...
	try 
		...
		let f = ref [(dis_to_exit ld (x,y) +. dist_euclidienne (x,y) start_coord, (x,y))] in
		...
		while !f <> [] do
			...
			then begin
				...
				List.iter (fun (ip, jp) -> 
						f := (dis_to_exit ld (ip, jp) +. dist_euclidienne (ip,jp) start_coord, (ip, jp)) :: !f;
...
let resolution_imprime (p : plateau) (x : int) (y : int) (_ : (int * int)) (ld : (int * int) list) : (int* int) list=
	let pere = Array.make_matrix (Array.length p) (Array.length p.(0)) (-1,-1) in

	let k = Array.length p.(0) -1 in 
	for j = 0 to Array.length p -1 do
		for i = 0 to k do
			set_color (rgb 150 150 150);
			draw_rect  (i*prop) ((k-j-1)*prop) prop prop;
			(match p.(j).(i) with
			| Obstacle -> set_color (rgb 0 0 0)
			| _ -> if j = y && i = x then set_color (rgb 255 0 0) 
					else set_color (rgb 250 250 250));
			fill_rect (i*prop+1) ((k-j-1)*prop+1) (prop-2) (prop-2)
		done;
	done;
	...
	with Solution -> 
		...
		List.iter (fun (a, b) -> 
				set_color (rgb 0 0 0);
				fill_rect ((min !xt a) * prop + prop/2 - t1/2) ((k-1-(max !yt b)) * prop + prop/2 - t1/2)
						(t1 + (abs (!xt - a)) * prop) (t1 + (abs (!yt - b)) * prop);
				(*set_color c.(i);
				fill_rect ((min !xt x) * prop + prop/2 - t2/2) ((n-1-(max !yt y)) * prop + prop/2 - t2/2)
						(t2 + (abs (!xt - x)) * prop) (t2 + (abs (!yt - y)) * prop);*)
				xt := a;
...
let _ = 
	...
	let p = init_matrix my mx (fun (x,y) -> match x, y with
													...
													|_, _ -> Empty) in
	(*let p = init_matrix my mx (fun (x,y) -> match x, y with
													|_, _ when x = mx-1 -> Obstacle
													|_, _ when x = 0 -> Obstacle
													|_, _ when y = my-1 -> Obstacle
													|_, _ when y = 0 -> Obstacle
													|_, _ when x = mx-3 && y < my -3 && y > 2 -> Obstacle
													|_, _ when y = my-3 && x < mx -2 && x > 1 -> Obstacle
													|_, _ when y = 2 && x < mx -2 && x > 1 -> Obstacle
													|_, _ -> Empty) in*)
	...
	(*let acoord = [|(1, my/2)|] in*)
	let astart_coord = Array.copy acoord in 
	...
	let achemin = Array.init (Array.length acoord) 
				(fun i -> resolution p (fst acoord.(i)) (snd acoord.(i)) astart_coord.(i) ld) in 
	...
	while Array.fold_left (+) 0 apeople <> -n && !temp > 0 do
		...
		for i = 0 to n-1 do
			...
			then begin  
				...
				| _ when List.mem acoord.(i) ld -> 
					...
					apeople.(i) <- -1
				| [] -> achemin.(i) <- resolution plateau (fst acoord.(i)) (snd acoord.(i)) astart_coord.(i) ld;
					if achemin.(i) <> [] 
					then begin 
		...
		while Sys.time () < start_time do () done;
