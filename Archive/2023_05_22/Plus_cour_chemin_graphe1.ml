...
let mouvement_valide (p : plateau) (pp : etat array array) (x : int) (y : int) : (int * int) list = 
	List.filter (fun (x,y) -> x >= 0 && y>=0 && y< Array.length p && x < Array.length p.(0) && (valide p x y) && pp.(y).(x) == Inconnu) (mouvement x y)

let mouvement_prioritaire_valide (p : plateau) (pp : etat array array) (x : int) (y : int) (ld: (int * int) list) : (int * int) list =
	sort (fun (a,b) (c,d) ->let minab = ref (Float.infinity) in 
							...
							!minab -. !mincd
							(*sqrt (float_of_int ((xd-a)*(xd-a) + (yd-b)*(yd-b))) -. sqrt (float_of_int ((xd-c)*(xd-c) + (yd-d)*(yd-d)))*)) (mouvement_valide p pp x y)
...
let resolution_flood_fill (p : plateau) (x : int) (y : int) (ld : (int * int) list) : (int* int) list=
	...
	try 
		...
		if !m > 0 then raise Solution
			...
			Printf.printf "La liste vide a ete donnee\n";
			[]
		end
	with Solution -> 
...
let _ = 
	...
	let p = init_matrix my mx (fun (x,y) -> match x, y with
													...
													|_, _ when x = mx-1 && y > my/2 +1 -> Obstacle
													(*|_, _ when x = mx-3 && y = my/2 -> Obstacle*)
													|_, _ -> Empty) in
	let ld = [(mx-1, my/2); (mx-1, my/2 -1); (mx-1, my/2 +1)] in
	List.iter (fun (xd, yd) -> p.(yd).(xd) <- Exit) ld;
	let n = 90 in
	let tmp = randomarray n (my*my) in
	let acoord = (*[|(4, 18)|]*)Array.init n (fun i -> tmp.(i)/my, tmp.(i) mod my) in
	let apeople = Array.init n (fun i -> i) in
	let acouleur = (*[|magenta|]*) Array.init n (fun i -> 
									let c = float_of_int (tmp.(i)) /. float_of_int (my*my) in 
									rgb (r c) (g c) (b c)) in
	let achemin = Array.init (Array.length acoord) (fun i -> resolution_flood_fill p (fst acoord.(i)) (snd acoord.(i)) ld) in 
	for i = 0 to n-1 do 
		let plateau = Array.init (Array.length p) (fun i -> Array.copy p.(i)) in
		print_map (applique_chemin (acoord.(i)::achemin.(i)) plateau (List.length achemin.(i) +1))
	done;
	let plateau = Array.init (Array.length p) (fun i -> Array.copy p.(i)) in
	for i = 0 to n-1 do 
