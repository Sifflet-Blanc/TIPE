...
let rec detection_pt_physique lpeople (obj : ((float*float)*(float*float)) list) =
	...
	let nobj = ((-1., Float.neg_infinity), (0., Float.infinity))::
		  ((maxX, Float.neg_infinity), (maxX+.1., Float.infinity))::
		  ((Float.neg_infinity, -1.), (Float.infinity, 0.))::
		  ((Float.neg_infinity, maxY), (Float.infinity, maxY+.1.)):: obj in 
	match lpeople with
	|p::q ->
		let orient_ray = ref 0. in
		let pt_vu = ref [] in
		...
		for _ = 0 to 249 do
			...
			let z = (sqrt((-2.) *. log(r1)) /. 3.899) *. cos(2. *. pi *. r2) in
			orient_ray := asin (z);
			(match raytracing_vecteur (p.direction +. !orient_ray) (fst p.position) (snd p.position) p.rayon nobj with
			|Some _,(x,y) ->
		...
		!pt_vu :: (detection_pt_physique q nobj)
	|[] -> []
