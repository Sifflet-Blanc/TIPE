...
let intersection_droite_rect a b o x y rep xp yp rp obji (change : bool) : bool =
...
let raytracing_vecteur (orient : float) (xp : float) (yp : float) (rp : float) (obj : ((float * float) * (float * float)) list) =
    ...
    let rec aux obj a b =
        ...
        |[] -> ()
        |obji::q ->
            ignore (intersection_droite_rect a b o x y rep xp yp rp obji true);
            aux q a b
        in
    aux obj a b;
...
let raytracing_vecteur_bvh (orient : float) (xp : float) (yp : float) (rp : float) (bvh : bvh) =
    let o = float_mod orient in
    let a = tan o in
    let b = yp -. a *. xp in
    let rep = ref (Some ((0., 0.), (0., 0.))) in
    ...
    let rec aux bvh : unit=
        match bvh with
        |Nil -> () 
        |Noeud (g, c, d) -> if intersection_droite_rect a b o x y rep xp yp rp c false
                            then begin 
                                aux g;
                                aux d;
                            end
        |Feuille c -> ignore (intersection_droite_rect a b o x y rep xp yp rp c true)
        in
    aux bvh;
    if !x = None || Option.get !x = Float.infinity || Option.get !x = Float.neg_infinity then begin
        Printf.printf "xp %f; yp %f; x %f; y %f \t o %f; dist %f\n*----------------------------------------------------------------------------------------------------*\n" xp yp (Option.get !x) (Option.get !y) o (dist_euclidienne (Option.get !x, Option.get !y) (xp, yp));
