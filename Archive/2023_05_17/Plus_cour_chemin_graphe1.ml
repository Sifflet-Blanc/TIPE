...
*)
let _ = 
	let p = init_matrix 20 20 (fun (x,y) -> match x, y with
													|_, _ when x = 18 && y < 10 -> Obstacle
													|_, _ when x = 18 && y > 10 -> Obstacle
													|_, _ -> Empty) in
	let xd, yd = 19, 10 in
	let apeople = [|(1, 1); (10, 15)|] in
	let nbactif = ref (Array.length apeople) in
	let achemin = Array.init (Array.length lpeople) (fun i -> resolution_flood_fill p (fst lpeople.(i)) (snd lpeople.(i)) xd yd) in 
	while !nbactif <> 0 do
		for i = 0 to !nb.actif-1 do 
			if p.(fst achemin.(i)).(snd achemin.(i)) = Empty 
			then 
