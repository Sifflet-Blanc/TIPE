...
let maxX : float = 10. (*largeur de la carte en metre*)
let maxY : float = 10. (*hauteur de la carte en metre*)
...
let people_init (l :float) (ep:float) (m:float) (pos: float * float) (allure_voulue:float) (dir:float) : people = 
	{largeur = l; epaisseur = ep; rayon = dist_euclidienne (l, ep) (0., 0.) /. 2.; masse = m; allure_voulue = allure_voulue; position = pos; allure = allure_voulue; direction = dir}
...
let changement_direction_allure (lp : people list) (ll : (float * (float * float) * float) list list) (obj : ((float * float) * (float * float)) list) : unit =
	(*	lp : liste des personne presente
 	...
 	let rec aux lp ll acc =
		...
		match lp,ll with 
		|p::qp,l::ql ->
			set_dir p (p.direction +. trouve_plus_grosse_somme_de_taille_m l 32 16);
...
let test_prog () = 
	...
	let ltest = [people_init 0.579 0.33 70. (1.399393, 0.498728) 0.5 0.;
	...
	let obj = [(*((-1., Float.neg_infinity), (0., Float.infinity));
		  ...
		  ((0.,0.),(maxX, maxY));
		  ((0.7 *. maxX, 0.), (0.75 *. maxX, 0.4 *. maxY));  ((0.7 *. maxX, 0.6 *. maxY), (0.75 *. maxX, 1. *. maxY))] in
	...
	while Sys.time () < start_time +. 50.0 do 
		...
		while Sys.time () < time +. (minimal_frame_time *. 0.5) do if key_pressed () then continue := false done;
   done;
   close_graph ()
...
let test_camera (dist : float) (pixel_w : float) = 
	...
	let ltest = [people_init 0.579 0.33 70. (4., 4.) 0.5 4.] in
	print_donne_people ltest;
...
let test_people_obj () = 
	...
	let test = people_init 0.579 0.33 70. (0.5, 0.5) 0. 0. in
