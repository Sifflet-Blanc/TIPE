...
let traitement_contact objl vx vy (p : people) (n : int): float * float =
	...
	let rayon_de_colision = p.rayon +. (p.allure +. acceleration_max_humain) *. dt in 
...
let changement_direction_allure2 (lp : people list) (ll : (float * (float * float) * float) list list) (obj : ((float * float) * (float * float)) list) (completobj : ((float * float) * (float * float)) list) : unit =
	(*	lp : liste des personne presente
		ll : liste de (liste des point vu par une personne) par personne
		obj : liste des objects sur le terrain *)
...
let test_prog () = 
	...
	(*let ltest = ref [people_init 0.579 0.33 70. (4., 18.) 0.5 (maxX -. 1., maxY /. 2.) (0. -. 0.6);
	...
	while !n > 0 do 
		...
		changement_direction_allure2 !ltest lo obj (!people_block @ obj);
...
let test_camera (dist : float) (pixel_w : float) = 
	...
	let ltest = [people_init 0.579 0.33 70. 0.5 (4., 4.) 4. (maxX -. 1., maxY /. 2.) 4.] in
	print_donne_people ltest;
	...
	while Sys.time () < start_time +. 50.0 do 
		...
		changement_direction_allure2 ltest lo obj (!people_block @ obj);
		while Sys.time () < time +. (dt *. 100.) do () done;
   done;
   close_graph ()
...
let test_people_obj () = 
	...
	let test = people_init 0.579 0.33 70. 0. (0.5, 0.5) 0. (0.5, 0.5) 0.90 in
